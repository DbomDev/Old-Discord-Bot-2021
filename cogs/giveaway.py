...
class Giveaway(commands.Cog):
    ...
    async def giveaway(self, ctx):
        ...
        giveawayEmbed = discord.Embed(
            ...
            description=f"Prize: {answers[2]}, Host: {ctx.author.mention}"
        )
        ...
        if len(users) == 0:
            ...
            #emWon.set_image(winnerIcon)
            emWon.set_thumbnail(url=winner.avatar_url)
            emWon.add_field(name = "Prize", description = f"{answers[2]}")
            emWon.set_footer(text="Giveaway Ended!")

            await winner.send(f"**Congrats {winner.mention}!**\nPlease contact {ctx.author.mention} about your prize.")
